...
function EditPackType() {
  ...
  const fetchBrand = async () => {
    await axios
      .get(process.env.REACT_APP_API_KEY + "singlePackType/" + id, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      ...
      .then((res) => {
        setInputs({
          ...
          valueSKU: res.data.data.SKU?.id,
          labelSKU: res.data.data.SKU?.name,
        });
  ...
  const validationSchema = Yup.object().shape({
    name: Yup.string()
      .required("PackType name is required")
      .min(3, "Minimum 3 Character")
      .max(80, "PackType name must not exceed 30 characters"),
    brandID: Yup.string().required("Brand is required"),
  });

  const formik = useFormik({
    initialValues: {
      name: inputs.packtype,
      brandID: inputs.value,
      hidden_value: inputs.packtype,
    },
    ...
    onSubmit: async (data) => {
      ...
      setIsLoading(true);
      await axios
        .post(process.env.REACT_APP_API_KEY + "update-packtype/" + id, data, {
          headers: {
            "Content-type": "Application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
            ...
            <div className="example-content">
              <form className="row g-3" onSubmit={formik.handleSubmit}>
                ...
                <div className="col-md-6">
                  ...
                  <Select
                    defaultValue={{
                      label: inputs.labelSKU,
                      value: inputs.valueSKU,
                    }}
                    name="skuID"
                    onChange={(selected) => {
                      formik.setFieldValue("skuID", selected.value);
                    }}
                    // options={data}
                    isSearchable={true}
                    isDisabled={true}

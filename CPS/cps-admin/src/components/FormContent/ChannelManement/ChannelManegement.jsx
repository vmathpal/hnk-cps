...
export default function ChannelManegement() {
  ...
  const getRoles = async () => {
    ...
    await axios({
      ...
      .catch(function (error) {
        swal({
          title: "Error!",
          text: "Region has already been assigned, you are not allowed to delete.",
          icon: "error",
          button: "Okay",
        });
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            await axios
              .delete(process.env.REACT_APP_API_KEY + "delete-channel/" + id, {
                headers: {
                  "Content-type": "Application/json",
                  Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
          ...
          <React.Fragment key={channel.id}>
            <tr>
              ...
              {localStorage.getItem("user_role") === "super_admin" ? (
                <td>
                  <NavLink
                    to={{ pathname: "/channel-management/edit-channel/"+channel.id }}
                    state={{ id: channel.id }}
                  >
      ...
      <>
        {localStorage.getItem("user_role") === "super_admin" ? (
          <NavLink
            ...
            to="/channel-management/add-channel"
          >
            Create Region
          </NavLink>
        ) : localStorage.getItem("user_role") === "sub_admin" &&
          ...
          <NavLink
            className="btn btn-info"
            to={{ pathname: "/channel-management/add-channel" }}
            state={{ action: action }}
          >
            Create Region
          </NavLink>

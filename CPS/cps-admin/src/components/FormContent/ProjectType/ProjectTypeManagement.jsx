...
export default function ProjectTypeManagement() {
  ...
  const projectType = async () => {
    setIsLoading(true);
    axios({
      url: process.env.REACT_APP_API_KEY + "project-type-lists",
      method: "get",
      params: {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            await axios
              .delete(
                process.env.REACT_APP_API_KEY + "delete-project-type/" + id,
                {
                  headers: {
                    "Content-type": "Application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
              ...
              .catch(function (error) {
                swal({
                  title: "Error!",
                  text: "Project type linked with other table, you are not allowed to delete.",
                  icon: "error",
                  button: "Okay",
                });
  ...
  const handleChange = async (id) => {
    await axios
      .get(process.env.REACT_APP_API_KEY + "project-type-status/" + id, {
        headers: {
          "Content-type": "Application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
            ...
            <tr>
              ...
              {localStorage.getItem("user_role") === "super_admin" ? (
                <td>
                  <NavLink
                    to={{
                      pathname: "/project-type-management/edit-project-type/"+volume.id
                    }}
                    state={{ id: volume.id }}
                  >
      ...
      <>
        {localStorage.getItem("user_role") === "super_admin" ? (
          <NavLink
            ...
            to="/project-type-management/add-project-type"
          >
            Create Project Type
          </NavLink>
        ) : localStorage.getItem("user_role") === "sub_admin" &&
          ...
          <NavLink
            className="btn btn-info"
            to={{ pathname: "/project-type-management/add-project-type" }}
            state={{ action: action }}
          >
            Create Project Type
          </NavLink>

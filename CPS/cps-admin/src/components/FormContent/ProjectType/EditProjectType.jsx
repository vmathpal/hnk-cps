...
function EditProjectType() {
  ...
  const projectTypes = () => {
    setIsLoading(true);
    axios
      .get(process.env.REACT_APP_API_KEY + "singleProjectType/" + id, {
        headers: {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (data) => {
      ...
      await axios
        .post(
          process.env.REACT_APP_API_KEY + "update-project-type/" + id,
          data,
          {
            headers: {
              "Content-type": "Application/json",
          ...
          if (res.status === 200) {
            ...
            setIsLoading(false);
            {
              localStorage.getItem("user_role") === "super_admin"
                ? navigate("/project-type-management")
                : navigate("/project-type-management/Modify");
            }
          }
        })
        .catch((error) => {
                  ...
                  <Link
                    ...
                    to={
                      localStorage.getItem("user_role") === "sub_admin"
                        ? "/project-type-management/Modify"

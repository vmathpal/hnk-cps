...
export default function LevelMapping5And6Listing() {
  ...
  const getRoles = async () => {
    setIsLoading(true);
    await axios
      .get(process.env.REACT_APP_API_KEY + "role-level56", {
        headers: {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            await axios
              .delete(
                process.env.REACT_APP_API_KEY + "delete-levelRole56/" + id,
                {
                  headers: {
                    "Content-type": "Application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const renderUser = (
    <table id="myTable" className="table table-striped my-3">
      <thead className="table-dark">
        <tr>
          <th scope="col">#</th>
          <th scope="col">Level 5 Role</th>
          <th scope="col">Level 6 Role</th>
          ...
          <React.Fragment key={role.id}>
            <tr>
              ...
              {localStorage.getItem("user_role") === "super_admin" ? (
                <td>
                  <NavLink
                    to={{ pathname: "/relation-mapping/edit-role-mapping56" }}
      ...
      <div className="row">
        <div className="col">
          <div className="page-description">
            <h1>Level 5 & 6 Relation Mapping Role Listing</h1>
          </div>
        </div>
      </div>
      <>

...
export default function LevelMapping4And5Listing() {
  ...
  const getRoles = async () => {
    setIsLoading(true);
    await axios
      .get(process.env.REACT_APP_API_KEY + "role-level45", {
        headers: {
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            await axios
              .delete(
                process.env.REACT_APP_API_KEY + "delete-levelRole45/" + id,
                {
                  headers: {
                    "Content-type": "Application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
          ...
          <React.Fragment key={role.id}>
            <tr>
              ...
              {localStorage.getItem("user_role") === "super_admin" ? (
                <td>
                  <NavLink
                    to={{ pathname: "/relation-mapping/edit-role-mapping45" }}
      ...
      <div className="row">
        <div className="col">
          <div className="page-description">
            <h1>Level 4 & 5 Relation Mapping Role Listing</h1>
          </div>
        </div>
      </div>
      <>
